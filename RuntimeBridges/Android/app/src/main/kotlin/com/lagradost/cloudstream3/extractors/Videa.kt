...
class Videa : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        while (true) {
            ...
            val videaXml = if (isXml) {
                ...
                val xsHeader = response.okhttpResponse.headers["X-Videa-Xs"] ?: return
                key += xsHeader
    ...
    private suspend fun getXmlUrl(url: String, cookieCallback: (String) -> Unit = {}): String? {
        ...
        response.okhttpResponse.headers["Set-Cookie"]?.let { cookieHeader ->
            """sl=([^;]+)""".toRegex().find(cookieHeader)?.let {
                cookieCallback(it.groupValues[1])
            }
        }
        ...
        val playerHtml = playerResponse.textLarge

        // Update cookie from player response
        playerResponse.okhttpResponse.headers["Set-Cookie"]?.let { cookieHeader ->
            """sl=([^;]+)""".toRegex().find(cookieHeader)?.let {
                cookieCallback(it.groupValues[1])
            }
        }
